(async () => {
    ...
    const showFile = (material) => {
        ...
        // Solution Logic
        solutionButton.classList.add('hidden-block');
        solutionButton.onclick = null;

        if (material.originalName.startsWith('U')) {
            const baseName = material.originalName.substring(1); // everything after 'U'
            // Find matching L file in the same topic
            const topic = selectedSubjectTopics.find(t => t.id === material.topicId);
            if (topic && topic.files) {
                const solutionFile = topic.files.find(f => f.originalName === 'L' + baseName);
                if (solutionFile) {
                    solutionButton.classList.remove('hidden-block');
                    solutionButton.onclick = () => showFile(solutionFile);
                }
            }
        }
    };
    ...
    const renderList = (filterText = '') => {
        ...
        const filtered = selectedSubjectTopics?.map(t => t.files).flat().filter(item => {
            if (item.name.startsWith('L')) return false;
            return query ? item.name.toLowerCase().includes(query) : true;
        }) ?? [];
