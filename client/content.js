...
const showFile = (material, solution = false) => {
    ...
    if (material.content[POST_SOLUTION_FILE_ID_KEY] != null) {
        solutionButton.classList.remove('hidden-block');
        if (solution) {
            solutionButton.innerText = 'Aufgabe anzeigen';
        } else {
            solutionButton.innerText = 'Lösung anzeigen';
        }
        solutionButton.onclick = () => showFile(material, !solution);
    }
};
